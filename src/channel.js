 ...
 *  // 创建一个通道端口
	...
	var sender = channelB.createSender(window)
...
Channel.prototype.listen = function(handle) {
    this.unlisten();
    var receiver = handle ? makeReceiver(handle) : handle;
    if (receiver) {
...
Channel.prototype.once = function(action, fn) {
    ...
    self._actions[action] = proxy;
};

var CALLBACK_ACTION = "__CALLBACK__";
...
Channel.prototype._recv = function(recv, source) {
    ...
    if (action === CALLBACK_ACTION) { // 回调
        var cdata = this._callbacks[recv.cid],
            rdata = recv.data || {};
        if (cdata) {
            var cb = cdata.cb;
